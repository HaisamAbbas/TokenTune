...
def test_settings():
    ...
    assert str(settings.chat_url) == "http://localhost:4000/"
    assert settings.chat_api_key == "None"
    assert str(settings.embed_url) == "http://localhost:8080/v1"
    assert settings.embed_api_key == "None"
